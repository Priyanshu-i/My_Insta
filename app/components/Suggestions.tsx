...
export default function Suggestions() {
    const [suggestions, setSuggestions] = useState<{ username: string; jobTitle: string; id: number }[]>([]);
        ...
        <div className="mt-4 ml-10">
            ...
            {suggestions.map(suggestion => (
                <div key={suggestion.id} className="flex items-center justify-between mt-3">
                    <img className= "h-10 rounded-full border p-[2px]"src={`https://i.pravatar.cc/150?img=${Math.floor(Math.random() * 70) + 1}`} alt=''/>
